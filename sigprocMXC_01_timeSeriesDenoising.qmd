...
As you can see, most spikes happen in quick succession, whereas the longer the
...
```{python}
_, kernel1 = make_gaussian(1, radius=100, fwhm=25)
spike_proba = gaussian_filter(spiky_signal, kernel1)

# plot the filtered signal (spike probability density)
plt.plot(spiky_signal, 'b', lw=.1, label='spikes')
plt.plot(spike_proba, 'r',label='spike p.d.')
plt.legend()
