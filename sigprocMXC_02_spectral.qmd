...
chirp_time, chirp_signal = chirp_mat['signal'][0], chirp_mat['signal'][0]
chirp_freq = chirp_mat['srate'].item()



win_len = int(chirp_freq/2)
win_count = int(len(chirp_signal)/win_len)
win_ends = np.arange(win_len, len(chirp_signal), win_len)

freq_max = chirp_freq/2 # Nyquist frequency
freq_count = win_count + 1
freq_bins = np.linspace(0, freq_max, freq_count)

man_spec = np.zeros((len(freq_bins), len(win_ends)))


taper = hann(win_len)
for win_idx in range(len(win_ends)):
    win_start = win_ends[win_idx] - win_len
    win_end = win_ends[win_idx]
    chunk = chirp_signal[win_start:win_end]
    trans = np.abs(fftpack.fft(taper * chunk)/win_count)**2
    print(trans.shape)
print(man_spec.shape)
```
